...
// (see scripts/create-logo-bucket.ts) and stamps the public URL onto
// companies.logo_url. Mirrors api/upload-candidate-photo.ts exactly, except
// the caller's company is resolved via company_users instead of candidates.
//
// Auth: caller sends their Supabase bearer token; the token's auth user must
// be a member of the company being updated. Env: SUPABASE_URL,
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { data: companyUser, error: cuErr } = await admin
    .from('company_users')
    .select('company_id')
    .eq('auth_user_id', authUserId)
    .maybeSingle();
  if (cuErr) return res.status(500).json({ error: cuErr.message });
  if (!companyUser) return res.status(404).json({ error: 'No company account for this session.' });
  ...
  const path = `${companyUser.company_id}.${ext}`;
  ...
  const { error: updateErr } = await admin.from('companies').update({ logo_url: url }).eq('id', companyUser.company_id);
